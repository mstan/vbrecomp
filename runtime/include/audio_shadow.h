 ...
 *
 * The gate that makes a "verified-enhancement shadow" SAFE per PRINCIPLES.md
 * ("Verified-Enhancement HLE Is Allowed; Load-Bearing HLE Is Not"). A
 * higher-fidelity re-render of a guest audio engine (here the VSU's 6-channel
 * wavetable unit, re-rendered in float free of the hardware's integer mix
 * scaling and S16 requantization) runs ALONGSIDE the canon VSU mix, never
 * replacing it. This verifier decides, from the running canon stream, whether
 * the shadow is proven enough to substitute, and reverts loudly the instant it
 * stops matching:
 *
 ...
 *
 * The canon (emulated VSU) output stays the verify oracle; the shadow is never
 * ground truth. If this verifier is wrong, the worst case is "we keep playing
 * the authentic hardware mix" — it cannot corrupt output.
 *
 * ── Attribution ───────────────────────────────────────────────────
 ...
 */
#ifndef VB_AUDIO_SHADOW_H
#define VB_AUDIO_SHADOW_H
